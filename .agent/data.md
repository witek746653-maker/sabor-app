...
## Автосводка по JSON (`data/*.json`)
<!-- AUTOGENERATED:START data_schemas -->
Таблица: **Файл | Top-level тип | Записей | Ключи/поля (срез)**

| Файл | Top-level тип | Записей | Ключи/поля (срез) |
|---|---|---:|---|
| `data/artworks.json` | `array<object>` | 8 | `artMovement`, `author`, `brandRelevance`, `comments`, `description`, `features`, `id`, `image`, `location`, `object_type`, ... (+5) |
| `data/extras-breakfast.json` | `object` | 1 | `categories` |
| `data/manifest.json` | `empty_file` | 0 | - |
| `data/menu-bar.json` | `array<object>` | 24 | `allergens`, `cardIngredients`, `comments`, `contains`, `description`, `i18n`, `id`, `image`, `ingredients`, `menu`, ... (+7) |
| `data/menu-config.json` | `object` | 9 | `bar`, `breakfast`, `fest`, `general`, `kids`, `kitchen`, `season`, `tea`, `wine` |
| `data/menu-kitchen.json` | `array<object>` | 174 | `allergens`, `contains`, `description`, `features`, `i18n`, `id`, `image`, `ingredients`, `menu`, `object_type`, ... (+6) |
| `data/menu-tea.json` | `array<object>` | 20 | `allergens`, `brewing`, `caffeine`, `comments`, `description`, `features`, `i18n`, `id`, `image`, `menu`, ... (+7) |
| `data/menu-wine.json` | `array<object>` | 36 | `alcoholContent`, `category`, `comments`, `description`, `features`, `grapeVarieties`, `i18n`, `id`, `image`, `menu`, ... (+11) |
| `data/questions-general.json` | `array<object>` | 46 | `audioBack`, `audioFront`, `description`, `hint`, `id`, `imageBack`, `imageFront`, `menu`, `section`, `status`, ... (+1) |
| `data/tea-comparison.json` | `array<object>` | 6 | `caffeine`, `general_features`, `group`, `icon`, `id`, `image`, `recommendation`, `sections` |
| `data/tools-registry.json` | `array<empty>` | 0 | - |
| `data/wine-comparison.json` | `array<object>` | 11 | `group`, `icon`, `id`, `sections`, `wines` |
<!-- AUTOGENERATED:END data_schemas -->
