...
- **Источник правды (Source of Truth)** — в нашем случае это JSON‑файлы в папке `/data`.

---

## 4) Как менять меню (Важно!)
1. Редактируйте только **JSON‑файлы** в папке `data/` (например, `menu-kitchen.json`).
2. **JSON — главный**: при деплое данные из файлов автоматически затирают старые данные в базе SQLite на сервере.
3. После деплоя всегда нажимайте **Ctrl+F5** в браузере.
